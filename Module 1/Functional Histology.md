...
```mermaid
flowchart TD
	A[Epithelial Tissues] --> B{Layers of cells?}
	B --> Simple
	B --> Compound
	Simple --> C{Cell Shapes?}
	Compound --> D{Change Shapes?}
	C --> Pseudostratified
	C --> Squamous
	C --> Cuboidal
	C --> Columnar
	D --> Stratified
	D --> Transitional
	Pseudostratified --> E{Forming Cilia?}
	E --> Ciliated
	E --> Non-Ciliated
	Stratified --> F{Cell Shapes?}
	F --> G[Stratified squamous]
	F --> H[stratified cuboidal]
	F --> I[stratified columnar]
	G --> J{Containing keratin?}
	J --> Keratinised
	J --> Non-keratinised
```
...
```mermaid
flowchart TD
	Epithelial --> A[Covering Epithelium]
	Epithelial --> B[Glandular Epithelium]
```
...
```mermaid
flowchart LR
	A[More cytoplasm] --> B[More organelle]
	B --> C[More secretion]
```
...
flowchart TD
	...
	A --> C[Supporting Connective Tissue]
	A --> D[Fluid Connective Tissue]
	B --> Loose
	B --> Dense
	C --> Cartilage
	C --> Bone
